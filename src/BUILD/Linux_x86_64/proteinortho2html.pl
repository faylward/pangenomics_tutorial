...
print '<h3>Proteinortho 6 : '.$ARGV[0].'</h3>';
...
print '<input type="button" onclick="filterTable(0)" value="search" id="searchbutton">';#<input type="button" onclick="filterTable(1)" value="exact search">
print '<br><span>The search for <span id="countspanA"></span><br>results in <b><span id="countspanB"></span></b> groups.</span></div><div class="middleA"><div class="sidewaysdiv">manage</div><div class="topnull"><input type="button" onclick="toggleCols();" value="toggle selected columns" id="toggleColsButton"><br><input type="button" onclick="togglePlus();" value="toggle all (+) and (-)" id="togglePlusButton"><br><input type="checkbox" id="toggleShowTrueNames" name="toggleShowTrueNames"
          onclick="toggleShowTrueNames();"><label id="toggleShowTrueNamesLabel" for="toggleShowTrueNames">show IDs only</label></div></div>
<div class="middleB"><div class="sidewaysdiv">select</div><select data-placeholder="Begin typing a name to filter..." multiple class="chosen-select" name="test" id="select">
';
	open(FILE,"<",$ARGV[0]);
	while(<FILE>){
		my $curline=$_;
